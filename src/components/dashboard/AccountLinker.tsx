...
import { Button } from '@/components/ui/Button';
import { Loader2, XCircle } from 'lucide-react';
import { useConnect as useWagmiConnect, useSignMessage, useDisconnect as useWagmiDisconnect } from 'wagmi';
import { useWallet } from '@solana/wallet-adapter-react';
import { useWalletModal } from '@solana/wallet-adapter-react-ui';
import { createChallenge } from '@/lib/auth-utils';
...
export default function AccountLinker({ accounts }: AccountLinkerProps) {
  const router = useRouter();
  const { data: session } = useSession();
  const [isLoading, setIsLoading] = useState<string | null>(null); // Can be 'ethereum', 'solana', 'unlink-ethereum', etc.
  const [error, setError] = useState<string | null>(null);
  ...
  // --- Solana Hooks ---
  const { publicKey: solanaPublicKey, signMessage: signSolMessage, connected: isSolanaConnected, disconnect: disconnectSolana } = useWallet();
  const { setVisible: setSolanaModalVisible, visible: isSolanaModalVisible } = useWalletModal();
  ...
  const handleEthereumLink = async (connector: any) => {
    setIsLoading('ethereum');
    setError(null);
    try {
      ...
      const response = await fetch('/api/auth/link-account', { /* ... API call ... */ });
      if (!response.ok) throw new Error(await response.text());
      router.refresh();
    } catch (err: any) {
      setError(err.message);
  ...
  useEffect(() => {
    const signAndLinkSolana = async () => {
      if (!solanaPublicKey || !signSolMessage) return;
      try {
      ...
      <div className="space-y-2">
        ...
        {isLinked('ethereum') ? (
          <div className="flex items-center justify-between rounded-md border border-secondary p-3">
            ...
            <Button onClick={() => handleUnlink('ethereum')} variant="ghost" size="sm" disabled={!!isLoading}>
          ...
          connectors.map((connector) => (
            <Button key={connector.id} onClick={() => handleEthereumLink(connector)} disabled={!!isLoading} className="w-full flex justify-center items-center gap-3" variant="outline">
              {isLoading === 'ethereum' ? <Loader2 className="h-5 w-5 animate-spin" /> : <><EthereumIcon /> Link {connector.name}</>}
            </Button>
          ))
        )}
        ...
        {isLinked('solana') ? (
          <div className="flex items-center justify-between rounded-md border border-secondary p-3">
            ...
            <Button onClick={() => handleUnlink('solana')} variant="ghost" size="sm" disabled={!!isLoading}>
              {isLoading === 'unlink-solana' ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4 text-red-500" />}
            </Button>
          </div>
        ) : (
          <Button onClick={handleSolanaLink} disabled={!!isLoading} className="w-full flex justify-center items-center gap-3" variant="outline">
