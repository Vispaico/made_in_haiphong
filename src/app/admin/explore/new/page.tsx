  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 max-w-2xl space-y-6">
        {/* --- All other form fields remain above --- */}
        ...
        {/* ... (category, title, address, description, body, and images sections remain the same) ... */}
        <div className="space-y-2">
          ...
          <select id="category" value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-md border border-secondary bg-background/50 p-2.5">
            {exploreCategories.map(cat => (<option key={cat.id} value={cat.slug}>{cat.name}</option>))}
          </select>
        </div>
        <div className="space-y-2">
