...
import { Star, Users, Home, ShieldCheck } from 'lucide-react';
        ...
        <div className="mt-8 grid grid-cols-1 gap-x-12 gap-y-8 md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-foreground/80">
              <div className="flex items-center gap-1.5"><Star className="h-5 w-5 text-yellow-400" fill="currentColor" /> <span className="font-semibold">4.9</span> <span className="text-foreground/60">(85 reviews)</span></div>
            </div>
            {(accommodation.maxGuests || accommodation.bedrooms) && (<div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-foreground/80">{accommodation.maxGuests && (<div className="flex items-center gap-1.5"><Users className="h-5 w-5" /><span>{accommodation.maxGuests} Guests</span></div>)}{accommodation.maxGuests && accommodation.bedrooms && (<span className="text-foreground/30">•</span>)}{accommodation.bedrooms && (<div className="flex items-center gap-1.5"><Home className="h-5 w-5" /><span>{accommodation.bedrooms} Bedrooms</span></div>)}</div>)}
            ...
            <div className="mt-8 flex items-center gap-4 rounded-lg border border-secondary bg-background p-4">
              ...
              <div>
                ...
                {accommodation.author.emailVerified && (<div className="flex items-center gap-1 text-xs text-green-600"><ShieldCheck className="h-4 w-4" /><span>Verified Host</span></div>)}
              </div>
            </div>
          ...
          <div className="md:col-span-1">
            <div className="sticky top-28 rounded-lg border border-secondary bg-background p-6 shadow-lg">
              <BookingRequest listingId={accommodation.id} pricePerNight={accommodation.price} />
